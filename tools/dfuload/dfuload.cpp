// From gabonator/LA104 tools/dfuload (MIT). Patched for Linux: writes are synchronous and the
// file data is flushed before the directory entry, so the DFU never sees the entry first.
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
...
uint32_t fill_clusters(FILE*fd, int fdi, struct bpb33* bpb)
{
    uint32_t clust_size;
    uint8_t *buf;
    uint32_t rdl;
...
int main(int argc, const char * argv[]) 
{
	...
	// writing operations
        uint32_t written = fill_clusters(source, fd, bpb);
        putulong(dirent.deFileSize, written);  // the DFU flags a size mismatch as .ERR
        fsync(fd);  // all data on the device before the DFU learns about the file
        usleep(200 * 1000);
