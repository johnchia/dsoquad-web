...
- **Throughput:** 15.6 frames/s (4096 samples × 3 channels), 0 bad frames. Host decoding costs 5 ms/frame, so the limit is the device. **Built but not yet flashed:** table-driven CRC and overlapped capture/send (re-arm right after the FIFO read). Flash and re-bench at the start of the next session; the M3 target is ≥ 20 fps.
- **Open issue: DFU flashing is unreliable.** The DFU reported `ERR` for every v0.2.0 attempt (raw `dfuload`, `dfuload` with O_SYNC/fsync + real file size, and a plain `mcopy` FAT copy), even for the known-good v0.1.1 image in the same session. Still, one attempt did program v0.2.0 (it runs), and an earlier attempt left APP1 broken (SYS version screen + continuous tone). What we know: an `ERR` entry stays listed until a real power cycle; the VM passthrough now survives the DFU's re-enumeration (it didn't before, so earlier results were never seen). **Next time:** start from a power-cycled, empty DFU disk each attempt; read the DSO's LCD message on ERR; try v0.1.1 vs v0.2.0 with the same method to separate image from method; compare against a mounted-vfat `cp` + `sync` (needs sudo) as the reference path.
