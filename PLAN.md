...
- [x] M6.3 Simulator DUTs, then the Analyzer view: frequency-response mode end to end in the simulator (sweep, auto-range, settle, plot, CSV). Done: `analyzer/duts.js` (through, RC, RLC, speaker behind R; the simulator adds a B gain and delay mismatch), `run.js` (per point: generator and rate from the plan and checked against STATE, auto-range both channels, settle, N readings of B/A repeated when they disagree), `plot.js`, `ui.js` (header switch Scope | Analyzer; the scope stands down while it's open). Readouts refine themselves: 3 extra points in each interval holding a −3 dB crossing or the peak, and a parabolic peak; the simulated RLC band-pass gives 4.300/5.890 kHz and a 5.031 kHz peak against 4.300/5.891/5.033 in theory. Tests: `tests/test_web_analyzer_sim.mjs` (whole sweeps through the simulator). **Hardware needs three leads** (wave out, A, B): the owner is getting a third.
